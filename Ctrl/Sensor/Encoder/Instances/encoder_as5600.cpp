...
EncoderAS5600Base::EncoderAS5600Base()
{
...
/*Sensor 有更好的解决办法*/
float EncoderAS5600Base::GetSensorAngle()
{
    return ((float) GetRawData() / (float) cpr) * _2PI;
}


uint16_t EncoderAS5600Base::GetRawData()
{
...
uint8_t EncoderAS5600Base::SpiCalcEvenParity(uint16_t value)
{
    uint8_t cnt = 0;
    uint8_t i;
